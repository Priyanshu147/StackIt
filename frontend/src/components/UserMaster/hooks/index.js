import { useEffect, useMemo, useState } from "react";
import {
  fetchAllUsers,
  deleteUser,
  deleteMultipleUsers,
} from "../../../utils/api";
import {
  CONFIRM_BOX_MESSAGES,
  DEFAULT_WINDOW_SIZE,
  DEFAULTS,
  SNACKBAR_DETAILS,
  SORTING_STATUS,
  TABLE_IDS,
  USER_MASTER_BASEURL,
  USER_MASTER_COLUMNS_SORTINGS,
  USER_MASTER_FORM_BASEURL,
} from "../../../utils/constants";
import { useDispatch } from "react-redux";
import { uiActions } from "../../../store/ui-slice";
import { wrapApiRequestHandler } from "../../../utils/helpers";
import { useLocation, useNavigate } from "react-router-dom";
import useConfirmBox from "../../shared/ConfirmBox/hooks";

let timer;
function useUserMaster() {
  const { onAskConfirmation } = useConfirmBox();

  const dispatch = useDispatch();
  const navigate = useNavigate();
  const location = useLocation();

  const [userData, setUserData] = useState([]);
  const [selectedUsers, setSelectedUsers] = useState([]);
  const [selectedUserForUpdate, setSelectedUserForUpdate] = useState(
    DEFAULTS.IS_FETCH_ACTIVE
  );
  const [searchText, setSearchText] = useState(null);
  const [isFetchActive, setIsFetchActive] = useState(DEFAULTS.IS_FETCH_ACTIVE);
  const [pageNo, setPageNo] = useState(1);
  const [totalRecords, setTotalRecords] = useState(0);
  const [windowSize, setWindowSize] = useState(DEFAULT_WINDOW_SIZE);
  const [showNextPage, setShowNextPage] = useState(false);
  const [showPrevPage, setShowPrevPage] = useState(false);
  const [columnSortings, setColumnSortings] = useState(
    USER_MASTER_COLUMNS_SORTINGS
  );

  const showBack = location.pathname !== USER_MASTER_BASEURL;
  const FILTER = useMemo(
    () => ({
      status: isFetchActive,
      searchText,
      pageNo,
      windowSize,
      columnSortings,
    }),
    [isFetchActive, searchText, pageNo, windowSize, columnSortings]
  );
  const fetchUsersHandler = async (filter) => {
    await wrapApiRequestHandler({
      apiCall: () => fetchAllUsers(filter),
      setLocalLoading: (loading) => dispatch(uiActions.setIsLoading(loading)),
      onSuccess: (result) => {
        setUserData(result.data);
        setShowNextPage(result?.isNextPage || false);
        setTotalRecords(result?.totalRecords || 0);
      },
    });
  };

  const deleteUserHandler = async (userData) => {
    onAskConfirmation(async () => {
      await wrapApiRequestHandler({
        apiCall: () => deleteUser(userData[TABLE_IDS.USER]),
        setLocalLoading: (loading) => dispatch(uiActions.setIsLoading(loading)),
        onSuccess: () => {
          fetchUsersHandler(FILTER);
          dispatch(
            uiActions.setSnackBar({ ...SNACKBAR_DETAILS.ON_DELETE_USER })
          );
          resetControles();
        },
      });
    }, CONFIRM_BOX_MESSAGES.DELETE_CONFIRMATION);
  };

  const deleteManyUserHandler = async () => {
    onAskConfirmation(async () => {
      await wrapApiRequestHandler({
        apiCall: () => deleteMultipleUsers(selectedUsers),
        setLocalLoading: (loading) => dispatch(uiActions.setIsLoading(loading)),
        onSuccess: async (data) => {
          fetchUsersHandler(FILTER);
          if (data.deletedCount > 1) {
            dispatch(
              uiActions.setSnackBar({
                ...SNACKBAR_DETAILS.ON_DELETE_USER,
                message: SNACKBAR_DETAILS.ON_DELETE_USERS.message.replace(
                  "__count__",
                  data.deletedCount
                ),
              })
            );
          } else {
            dispatch(
              uiActions.setSnackBar({ ...SNACKBAR_DETAILS.ON_DELETE_USER })
            );
          }
          resetControles();
        },
      });
    }, CONFIRM_BOX_MESSAGES.DELETE_CONFIRMATION);
  };

  useEffect(() => {
    fetchUsersHandler(FILTER);
  }, [FILTER]);

  const selectUsersHandler = setSelectedUsers;
  const toggleSwitchHandler = (status) => {
    setPageNo(1);
    setIsFetchActive(status);
  };
  const searchUserHandler = (searchString) => {
    clearTimeout(timer);
    timer = setTimeout(() => {
      setSearchText(searchString.trim() || null);
      setPageNo(1);
      setShowPrevPage(false);
      setShowNextPage(false);
    }, 300);
  };
  const openUpdateFormHandler = setSelectedUserForUpdate;

  const toggleFormHandler = (isOpenForm, isFetch) => {
    if (!isOpenForm || isFetch) {
      resetControles();
      setIsFetchActive(DEFAULTS.IS_FETCH_ACTIVE);
      setSearchText(null);
    }

    if (isFetch && pageNo === 1 && isFetchActive && searchText === null) {
      fetchUsersHandler(FILTER);
    }
    navigate(isOpenForm ? USER_MASTER_FORM_BASEURL : USER_MASTER_BASEURL);
  };

  const changePageHandler = (pageNo) => setPageNo(pageNo);

  const nextPageHandler = () => {
    setPageNo((prevPage) => prevPage + 1);
    if (pageNo >= 1) {
      setShowPrevPage(true);
    }
  };
  const prevPageHandler = () => {
    setPageNo((prevPage) => prevPage - 1);
    if (pageNo <= 2) {
      setShowPrevPage(false);
    }
  };

  const windowSizeChangeHandler = (size) => {
    setWindowSize(size);
    resetControles();
  };

  const resetControles = () => {
    setSelectedUsers([]);
    setPageNo(1);
    setShowNextPage(false);
    setShowPrevPage(false);
    setColumnSortings(USER_MASTER_COLUMNS_SORTINGS);
  };

  const sortingOrderChangeHandler = (fieldName) => {
    setColumnSortings((prevState) => {
      const newSorting = {
        [fieldName]:
          prevState[fieldName] === SORTING_STATUS.ASCENDING
            ? SORTING_STATUS.DESCENDING
            : SORTING_STATUS.ASCENDING,
      };

      // Remove the field from the previous state and place it at the beginning
      // we did this so that latest changed column given priority in sorting
      const updatedState = { ...prevState };
      delete updatedState[fieldName];

      setColumnSortings({
        ...newSorting,
        ...updatedState,
      });
    });
  };

  const userState = {
    userData,
    onDeleteUser: deleteUserHandler,
    onDeleteManyUser: deleteManyUserHandler,
    onSelectUser: selectUsersHandler,
    onFetchUsers: fetchUsersHandler,
    selectedUsers,
    onToggleSwitch: toggleSwitchHandler,
    onEditUser: openUpdateFormHandler,
    showDelete: selectedUsers?.length > 0 && !showBack,
    selectedUserForUpdate,
    onSearch: searchUserHandler,
    columnSortings,
    onChangeSortingOrder: sortingOrderChangeHandler,
  };
  const formState = {
    showBack,
    onToggleForm: toggleFormHandler,
  };
  const pageState = {
    pageNo,
    showNextPage,
    showPrevPage,
    onChangePageNo: changePageHandler,
    onNextPage: nextPageHandler,
    onPrevPage: prevPageHandler,
    windowSize,
    totalRecords,
    onWindowSizeChange: windowSizeChangeHandler,
  };
  return [userState, formState, pageState];
}

export default useUserMaster;
